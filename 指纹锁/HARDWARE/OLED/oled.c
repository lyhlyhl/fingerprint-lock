...
{
/****************************IIC_SDA初始化***********************************************/
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_OLED_SDA_PORT, ENABLE);	//使能SDA连接端口时钟   /
	GPIO_OLED_InitStructure.GPIO_Pin = GPIO_Pin_OLED_SDA;										//SDA端口配置					   /
	GPIO_OLED_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 					//推挽输出							 /
	GPIO_OLED_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;							//IO口速度为50MHz				 /
	GPIO_Init(GPIO_OLED_SDA_PORT, &GPIO_OLED_InitStructure);					 			//根据设定参数初始化SDA /
/****************************IIC_SCL初始化***********************************************/
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_OLED_SCL_PORT, ENABLE);	//使能SCL连接端口时钟 	 /
	GPIO_OLED_InitStructure.GPIO_Pin = GPIO_Pin_OLED_SCL;										//SCL端口配置						 /
	GPIO_OLED_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 					//推挽输出							 /
	GPIO_OLED_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;							//IO口速度为50MHz				 /
	GPIO_Init(GPIO_OLED_SCL_PORT, &GPIO_OLED_InitStructure);					 			//根据设定参数初始化SCL /
/****************************************************************************************/
}
...
{
	GPIO_OLED_InitStructure.GPIO_Pin = GPIO_Pin_OLED_SDA;										//SDA端口配置					   /
	GPIO_OLED_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 					//推挽输出							 /
	GPIO_Init(GPIO_OLED_SDA_PORT, &GPIO_OLED_InitStructure);					 			//根据设定参数初始化SDA /
}
void OLED_SDA_OUT()
{
	GPIO_OLED_InitStructure.GPIO_Pin = GPIO_Pin_OLED_SDA;										//SCL端口配置						 /
	GPIO_OLED_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 					//推挽输出							 /
	GPIO_Init(GPIO_OLED_SDA_PORT, &GPIO_OLED_InitStructure);					 			//根据设定参数初始化SCL /
}
...
void OLED_IIC_Write_Data(unsigned char Device_addr,unsigned char Reg_addr,unsigned char data)
{
  OLED_IIC_Start();
	OLED_SDA_OUT();
  OLED_IIC_Write_Byte(Device_addr | 0x00);	//器件地址(7位) + 读写位(读1写0)
	OLED_IIC_Wait_Ack();	
  OLED_IIC_Write_Byte(Reg_addr);						//要写入数据的寄存器地址
	OLED_IIC_Wait_Ack();	
...
unsigned char OLED_IIC_Read_Data(unsigned char Device_addr,unsigned char Reg_addr,unsigned char ACK_EN)
{
	...
	OLED_IIC_Start();
  OLED_IIC_Write_Byte(Device_addr | 0x00);	//器件地址(7位) + 读写位(写0)
	OLED_IIC_Wait_Ack();	
  OLED_IIC_Write_Byte(Reg_addr);						//要读数据的寄存器地址
	OLED_IIC_Wait_Ack();	
  OLED_IIC_Start();
	OLED_SDA_IN();
  OLED_IIC_Write_Byte(Device_addr | 0x01);	//器件地址(7位) + 读写位(读1)
	OLED_IIC_Wait_Ack();	
...
void OLED_Init()
{
	...
	OLED_WriteCmd(0xa1); //---设置段重新映射，0~127
